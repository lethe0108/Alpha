...
console.log('📁 尝试访问 Alpha 目录并创建文档...\n');
console.log('Alpha 目录：https://xvgo1faf8xg.feishu.cn/drive/folder/' + alphaFolderToken);
console.log('');

// 先测试访问权限
console.log('【测试】访问 Alpha 目录...');
client.drive.file.list({
  params: { folder_token: alphaFolderToken }
}).then((listRes) => {
  if (listRes.code === 0) {
    console.log('✅ 可以访问 Alpha 目录');
    console.log('   目录中有 ' + (listRes.data?.files?.length || 0) + ' 个文件/文件夹\n');
    
    // 创建文档到根目录（不指定 folder_token）
    console.log('【创建】创建文档到云盘根目录...\n');
    createDocsToRoot();
  } else {
    console.log('❌ 无法访问 Alpha 目录：' + listRes.msg);
    console.log('   错误代码：' + listRes.code);
    console.log('\n建议：请在飞书云盘中手动将应用添加为协作者');
  }
}).catch((e) => {
  console.log('❌ 异常：' + e.message);
});

async function createDocsToRoot() {
  const docs = [
  ...
  console.log('\n' + '='.repeat(70));
  console.log('✅ 完成！创建 ' + createdDocs.length + ' 个文档到云盘根目录');
