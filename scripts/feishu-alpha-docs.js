...
console.log('📁 在 Alpha 目录创建所有项目文档\n');
...
async function run() {
  for (const doc of docs) {
    ...
    try {
      const docId = await createAndWrite(doc.title, doc.content, alphaFolderToken);
  ...
  console.log('\n' + '='.repeat(70));
  console.log('✅ 完成！创建 ' + createdDocs.length + '/' + docs.length + ' 个文档');
...
function saveToMemory() {
  const memory = {
    ...
    rule: '所有项目文档自动创建到 Alpha 目录 (O4REfrwt1lSbRUd7ha0cLyxinVb)'
  };
