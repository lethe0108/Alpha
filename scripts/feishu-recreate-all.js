...
console.log('📚 重新创建完整项目文档到 Alpha 目录\n');
...
async function run() {
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ 创建完成！成功 ' + createdDocs.length + '/' + docs.length);
...
async function createAndWrite(title, markdown) {
  // 1. 创建文档 - 使用 folder_token 参数
  const createRes = await client.docx.document.create({
    data: { 
      title: title,
      folder_token: alphaFolderToken  // 关键：使用 folder_token
    }
  });
  ...
  if (insertRes.code !== 0) throw new Error('插入失败：' + insertRes.msg);
    ...
    res.on('end', () => {
      ...
      if (result.data && result.data.files) {
        ...
        if (ourDocs.length > 0) {
          ...
          console.log('\n✅ 成功！共计 ' + ourDocs.length + ' 个文档在 Alpha 目录中');
        } else {
          console.log('  ❌ 没有找到新创建的文档');
          console.log('  目录中所有文件:');
          result.data.files.slice(0, 10).forEach(f => {
