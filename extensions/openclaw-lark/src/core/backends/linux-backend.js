 ...
 *
 * Linux encrypted file backend for persistent token storage.
...
const log = larkLogger('core/backends/linux');

// Storage directory
const STORAGE_DIR = path.join(process.env.HOME || '/root', '.local', 'share', 'openclaw-feishu-uat');

// Encryption key - load from persistent master key file
const MASTER_KEY_FILE = path.join(STORAGE_DIR, 'master.key');

function loadEncryptionKey() {
  try {
    const key = fs.readFileSync(MASTER_KEY_FILE);
    log.debug('loaded encryption key from master.key');
    return key;
  } catch (error) {
    if (error.code === 'ENOENT') {
      log.warn('master.key not found, generating and saving new key');
      const newKey = crypto.randomBytes(32);
      try {
        fs.mkdirSync(path.dirname(MASTER_KEY_FILE), { recursive: true, mode: 0o700 });
        fs.writeFileSync(MASTER_KEY_FILE, newKey, { mode: 0o600 });
        log.info('created new master.key');
      } catch (saveErr) {
        log.error(`failed to save master.key: ${saveErr.message}`);
      }
      return newKey;
    }
    log.error(`failed to load master key: ${error.message}`);
...
async function ensureStorageDir() {
  try {
    await fsPromises.mkdir(STORAGE_DIR, { recursive: true, mode: 0o700 });
  } catch (error) {
...
export async function saveToken(key, data) {
  try {
    ...
    await fsPromises.writeFile(filePath, encrypted, { encoding: 'utf8', mode: 0o600 });
...
async function initialize() {
  await ensureStorageDir();
  log.debug('linux file backend initialized');
}
