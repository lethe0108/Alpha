...
import { stripIncompleteMediaTag, findFirstClosedMediaTag, executeSendQueue, } from "./utils/media-send.js";
// ============ 常量 ============
/** 流式消息节流常量（毫秒） */
...
export class StreamingController {
    ...
    _callbackChain = Promise.resolve();
    // ---- 互斥：首个到达的回调锁定控制权 ----
    /**
        ...
        try {
            ...
            while (true) {
                ...
                if (found.mediaPath && this.deps.mediaContext) {
                    const item = { type: found.itemType, content: found.mediaPath };
    ...
    async sendStreamChunk(content, inputState, caller) {
        ...
        const token = await getAccessToken(this.deps.account.appId, this.deps.account.clientSecret);
 ...
 */
export function shouldUseStreaming(account, targetType) {
